...
final public class Query4<C0: QueryTarget, C1: QueryTarget, C2: QueryTarget, C3: QueryTarget>: Chunk, SystemParameter {
    var components = SparseSet<(Ref<C0>, Ref<C1>, Ref<C2>, Ref<C3>)>(sparse: [], dense: [], data: [])
    ...
    public func update(_ f: (inout C0, inout C1, inout C2, inout C3) -> ()) {
        for ref in self.components.data {
            f(&ref.0.value, &ref.1.value, &ref.2.value, &ref.3.value)
        }
    }
    
    public func update(_ entity: Entity, _ f: (inout C0, inout C1, inout C2, inout C3) -> ()) {
